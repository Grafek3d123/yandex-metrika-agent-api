...
## 0. Быстрый контекст (читать первым)

Чтобы вернуть контекст после потери (новая сессия) или перед ответом на новый
вопрос, агенту достаточно прочитать ядро — это 1–2 файла. Остальное читать
выборочно под задачу.

**Ядро (всегда, при любом старте/новом вопросе):**
1. `KODA.md` — этот файл: назначение, архитектура, команды, правила, статус.
2. `pyproject.toml` — зависимости, консольный скрипт, конфиги ruff/mypy/pytest
   (быстро сверить версии/настройки, не доверяя памяти).

**По задаче (читать, когда запрос касается темы):**
- Правка/обзор кода, «что уже есть» → `docs/yandex-metrika-ai.md` (детальный
  реестр реализованного; раздел §19 — официальные ссылки на документацию Яндекса).
- Активная задача/этап → `Tasks/TaskNN.md` (ТЗ) и `Tasks/TaskNN_REPORT.md` (отчёт).
- Настройки/env → `.env.example` (фактический `.env` не коммитится).
- Целевой CLI/API (с известным расхождением — см. §8) → `README.md`.
- Конкретный модуль → сам модуль `yandex_metrika_agent/<mod>.py` + его тест
  `tests/test_<mod>.py` (тесты — исполнимая спецификация поведения).

**Порядок при потере контекста:** `KODA.md` → (нужен статус фич) `docs/…md` →
(есть активная задача) `Tasks/TaskNN*.md` → целевой модуль + тест.
...
AI Tool Layer (ai_tools/): MetrikaTools — 14 инструментов + confirmation guard
   ↓
...
| `ai_tools/` | AI Tool Layer: пакет инструментов со строгими JSON-схемами. `base.py` (реестр, `Tool`, `ToolSafety`, confirmation guard), `counters.py`/`goals.py`/`analytics.py` (14 инструментов `metrika_*`), `__init__.py` (фасад `MetrikaTools`). |
| `log.py` | Логирование (анонимизация чувствительных данных). |
| `__init__.py` | Публичный API пакета (ошибки, модели, планировщик, фасад `MetrikaTools`). |
...
> ✅ **187 unit-тестов** (pytest + pytest-asyncio + respx, моки HTTP — без реальных
> credentials). Покрывают транспорт, клиент, OAuth, токены/крипто, счётчики, цели
> (+ destructive-safety guard), отчёты, метрики, фильтры, планировщик и AI Tool Layer.
> Плюс ручные прогоны (`compileall`, `mypy`, CLI).

### 5.1 ⚠️ Запуск в Windows PowerShell: кодировка и кавычки

Окружение — Windows + **PowerShell 5.1** (не cmd, не PowerShell 7). Два вечных грабля:

**А. Кракозябры вместо кириллицы в выводе.**
PowerShell 5.1 пишет в консоль в OEM-кодировке (cp866/cp1251), а Python отдаёт UTF-8 →
кириллица в stdout превращается в `?????`/`▒▒▒`. Это **только cosmetic**: сам Python и
данные корректны, ломается лишь чтение из терминала. Как снимать:
- в любом скрипте, печатающем кириллицу, первой строкой:
  `sys.stdout.reconfigure(encoding="utf-8")` (Python ≥ 3.7);
- либо перенаправлять вывод в файл и читать его read-инструментом:
  `python scripts/x.py | Out-File -Encoding utf8 reports/_out.txt` (потом удалить);
- файлы на запись писать явно: JSON/текст — `encoding="utf-8"`, CSV для Excel —
  `encoding="utf-8-sig"` (иначе Excel покажет кракозябры).
- **Не доверять** чтению кириллического stdout напрямую — всегда reconfigure или файл.

**Б. PowerShell ломает `python -c "..."` с кавычками/спецсимволами.**
PS 5.1 по-своему парсит `"`, `$`, `` ` ``, `!`, `{}` → многострочный код с f-строками,
словарями, `!r` падает с `ScriptBlock следует указывать только в качестве значения для
параметра Command`. Правила:
- **не запускать многострочный Python через `-c "..."`** — писать код в отдельный
  временный `.py`-файл (например `scripts/_inspect.py`) и запускать его;
- одноразовый однострочник — внешние **одинарные** кавычки PS, двойные внутри Python:
  `python -c 'print("ok")'`;
- внутри `-c` не использовать `$`, `` ` ``, `!`, `;` и f-строки с `{}`.

Оба пункта — про **оболочку**, а не про код пакета: сам `yandex_metrika_agent` кроссплатформенный.
...
- **Dev-зависимости** — в `[dependency-groups]`, а не в extras `[dev]`, поэтому
  ...
  вручную).
- **Тесты есть** — 187 unit-тестов в `tests/` (pytest + respx). Интеграционных (на живой API) нет.
...
метрик; DSL фильтров; нормализованные ошибки; **AI Tool Layer** (пакет `ai_tools/`:
14 инструментов `metrika_*` со строгими JSON-схемами + фасад `MetrikaTools` +
confirmation guard для разрушающих операций); **187 unit-тестов**; CLI `plan`.

**В планах (не реализовано):** полный CLI из README; интеграционные тесты на живой
API; расширения (Logs API, Segments, Imports, offline conversions).
...
## 9. Перед каждым коммитом

### 9.1 Файлы, которые надо синхронизировать (обновить под внесённые изменения)

| Файл | Обновлять, если изменилось |
| --- | --- |
| `KODA.md` | карта модулей (§3), команды/число тестов (§5), статус «реализовано/планы» (§8) |
| `docs/yandex-metrika-ai.md` | реестр реализованного, счётчики (тесты/инструменты), карта модулей, roadmap |
| `README.md` | публичный CLI/API (учесть расхождение §8: README описывает целевой CLI) |
| `pyproject.toml` | новая зависимость / dev-группа / консольный скрипт / конфиг линтера |
| `.env.example` | новая переменная окружения (фактический `.env` не коммитить) |
| `Tasks/TaskNN_REPORT.md` | итоги завершённого этапа/задачи |

Правило: код и документы расходятся незаметно. Любая правка модуля/метрик/CLI/env
или изменение числа тестов обязана отразиться в соответствующей строке таблицы выше
в том же коммите. Число тестов и число инструментов — сверять прогоном
(`pytest --co -q`, `len(ALL_TOOLS)`), а не по памяти.

### 9.2 Проверки качества (обязательны до коммита)

```bash
pytest                                   # все тесты зелёные (сейчас 187)
mypy                                     # строгая проверка типов
ruff check yandex_metrika_agent          # линтер
python -m compileall yandex_metrika_agent
```

### 9.3 Чистота структуры проекта (не оставлять мусор)

Перед `git add` прогнать `git status` и убрать временное. В репозитории не должно
быть файлов, не относящихся к библиотеке/докам/задачам:

- **Временные скрипты/инспекторы** — только с префиксом `_` в `scripts/`
  (уже игнорируются: `scripts/_*.py`) и **удалять сразу после использования**.
- **Временный вывод** (`*_out.txt`, `_check_out.txt`, дампы) — не оставлять, удалять.
- **Артефакты приёмки** (`scripts/*_results.json`, `*_run.log`) — не коммитить
  (в них id живых счётчиков); убедиться, что они в `.gitignore`.
- **Клиентские данные** — `reports/` игнорируется целиком (конфиденциально).
- **Кэши/venv** (`__pycache__/`, `.venv/`, `.mypy_cache/`, `.ruff_cache/`) — игнорируются.
- **`git add` — точечно** (по файлам), не `git add -A` вслепую: это не даст мусору
  просочиться в индекс.

> ⚠️ **Известный мусор в истории:** `scripts/task05_results.json` уже отслеживается
> git (в отличие от аналогов `task03_*`, которые в `.gitignore`). Это артефакт
> приёмки Task05 с id живых объектов — при случае удалить из индекса
> (`git rm --cached`) и добавить в `.gitignore`.

---

## 10. Как использовать содержимое
...
- **Запуск скриптов в PowerShell:** сначала §5.1 — кириллицу в stdout выводить только
  через `sys.stdout.reconfigure(encoding="utf-8")` или в файл; не гонять многострочный
  `python -c "..."` (PowerShell ломает кавычки) — писать временный `.py`.
